...
function initIntersectionObserver() {
    ...
    document.querySelectorAll(
        ".service-card,.about-card,.about-content,.stat,.contact-card,.hiw-card,.hiw-step-4"
    ).forEach(el => {
        observer.observe(el);
    });
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // ---- Attach event listeners for buttons (no inline onclick) ----
    // Hero buttons
    ...
    // Section dots – they use data-target and we attach click listener
    document.querySelectorAll(".dot").forEach(dot => {
    ...
    // Any other anchor links with data-section (desktop nav) – already have href, but we can add smooth scroll via JS
    document.querySelectorAll(".nav-link").forEach(link => {
    ...
    // Footer quick links also have data-section; we add smooth scroll for those too
